		...
		var laypageindex = laypage({
		  ...
		  jump: function(obj, first) {
			if(!first) {
			  ...
			  GetList("Admin-Goods-ajaxGetType")
			}
		  }
		})
	  }
...
init_html=function (data){
	...
	$.each(data,function(index, object) {
		_html += "<tr class='poid'  poid='"+object.id+"'><td align='center'>"+object.id+"</td>"+
					"<td align='center'>"+object.type_name+"</td>"+
					"<td align='center'>"+
					' <a href="Admin-Goods-getAttr-typeId-'+object.id+'" class="btn btn-darkorange btn-sm"><i class="fa fa-cog"></i>属性列表</a>'+
					'<a class="btn btn-primary btn-sm shiny" href=Admin-Goods-addType-typeId-'+object.id+'> <i class="fa fa-edit"></i>编辑</a>'+
  ...
  _this.init("Admin-Goods-ajaxGetType");

  Posts.on('click','.delete',function(event) {
  	//var adminId = .push();
  	typeId = $(this).parent().parent('.poid').attr('poid');
	layer.confirm('真的要删除吗？', {
	  btn: ['确定', '在想想'] //可以无限个按钮
	}, function(index, layero){
	 $.ajax({
	 	url: 'Admin-Goods-ajaxDelType',
	 	type: 'post',
	 	dataType: 'json',
	 	data: {typeId: typeId},
	 	success:function(data){
